...
#if 0   // 选择满足电压<=Sink最大电压的Source能力中功率最大者
...
#else   // 直接根据Source能力索引选择
    extern int source_capability_index;
    objVoltage = port->SrcCapsReceived[source_capability_index].FPDOSupply.Voltage * 50;
    if (source_capability_index < port->SrcCapsHeaderReceived.NumDataObjects && 
        objVoltage <= port->PortConfig.SinkRequestMaxVoltage)
    {
        SelVoltage = objVoltage;
        reqPos = source_capability_index + 1;
    }
#endif
